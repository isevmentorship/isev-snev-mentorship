...
strongest at mentoring on. Ranking is the primary matching signal after
locality.
...
| `secondary_topic_weight` | 0.30 | Multiplier applied to mentee secondary-tier topic overlap in §6.2 |
...
2. `apply.html` presents the full form. After choosing a role, they pick
   topic areas (mentees rank up to 5 primary priorities and optionally up to 5
   secondary interests per §3.1; mentors rank up to 5 topics they can mentor
   on), tag focus areas, select their time zone from a dropdown, acknowledge
   the professional-development framing, consent to review and unblinding, and
   submit.
...
**Rank weight table** (both sides use the same weights when they have ranks):
...
Weights sum to 1.00 so a full five-for-five match in a single tier produces a
perfect 1.00.
